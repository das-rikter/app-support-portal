...
export function PriorityCharts() {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (c) =>
                ` ${c.parsed} bugs (${Math.round(((c.parsed as number) / pTotal) * 100)}%)`,
            },
          },
        },
      },
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: {
              color: '#1a202c',
              font: { family: "'Satoshi', sans-serif", size: 9 },
              maxRotation: 35,
            },
  ...
  return (
    <section id="section-priority" className="report-section flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-5 max-[900px]:grid-cols-1">
        <div className="bg-white rounded-xl border border-[var(--br-border)] shadow-sm p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 pt-3 border-t border-[var(--br-border)]">
            {prioLabels.map((label, i) => (
              <div key={label} className="flex items-center gap-2 text-xs">
                <span
                  className="w-2.5 h-2.5 rounded-full shrink-0"
                  style={{ background: PRIO_COLORS[i] }}
                />
                <span className="flex-1 text-[var(--br-text-muted)]">{label}</span>
                <span className="tabular-nums font-bold text-[var(--br-text)]">
                  {prioCounts[i]}
                </span>
                <span className="text-[var(--br-text-faint)] text-[0.7rem]">
        ...
        <div className="bg-white rounded-xl border border-[var(--br-border)] shadow-sm p-6 flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-[var(--br-text-muted)]">
              Stacked breakdown of priority per project
            </span>
          </div>
